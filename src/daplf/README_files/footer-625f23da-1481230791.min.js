...
else{if(typeof(this.donutchartsettings)=="object")
...
$(this).one('inview',function(event,visible){if(visible===!0){var val=$(this).find('.dt-sc-counter-number').attr('data-value');$(this).find('.dt-sc-counter-number').animateNumber({number:val,numberStep:$append},2000)}})});if($('ul.dt-sc-vertical-nav').length){var $top,$tmin;if(dttheme_urls.stickynav==="enable"){$top=80;$tmin=132}else{$top=0;$tmin=52}
...
$('.dt-sc-subscribe-frm').each(function(){$(this).submit(function(){var This=$(this);var $data="";var $name=$(this).find('input[name=mc_name]').val();var $email=$(this).find('input[name=mc_email]').val();var $listid=$(this).find('input[name=mc_listid]').val();$.ajax({type:"post",dataType:"html",url:dttheme_urls.ajaxurl,data:{action:"dt_ajax_mc_subscribe",mc_name:$name,mc_email:$email,mc_listid:$listid},success:function(data){$data=$(data);if($data.length>0){This.next('.dt_ajax_subscribe_msg').html($data)}},error:function(jqXHR,textStatus,errorThrown){This.next('.dt_ajax_subscribe_msg').html('Page load faild.')}});return !1})});$(window).load(function(){if($(".carousel_items").length){$(".carousel_items .dt-sc-testimonial-carousel").each(function(){var $prev=$(this).parents(".carousel_items").find(".testimonial-prev");var $next=$(this).parents(".carousel_items").find(".testimonial-next");var $anim=$(this).parents(".carousel_items").attr("data-animation");$(this).carouFredSel({responsive:!0,auto:!1,width:'100%',prev:$prev,next:$next,height:'variable',scroll:{easing:"linear",duration:500,fx:$anim},items:{width:1170,height:'variable',visible:{min:1,max:1}}})})}
if($(".dt-sc-partners-carousel").length){$(".dt-sc-partners-carousel").each(function(){var $prev=$(this).parents(".dt-sc-partners-carousel-wrapper").find(".partners-prev");var $next=$(this).parents(".dt-sc-partners-carousel-wrapper").find(".partners-next");var $scroll=$(this).parents(".dt-sc-partners-carousel-wrapper").attr('data-scroll');var $visible=$(this).parents(".dt-sc-partners-carousel-wrapper").attr('data-visible');$(this).carouFredSel({responsive:!0,auto:!0,width:'100%',height:'variable',prev:$prev,next:$next,scroll:parseInt($scroll),items:{visible:{min:parseInt($visible)}}})})}
